...
class IntentClassifier:
    ...
    def classify(self, user_input: str) -> dict:
        ...
        industry_id = top_industry.id if top_industry else None
        ...
        return {
            ...
            "matched_industries": [
                {"id": m.id, "name": m.name, "score": round(self._normalize_score(m.score, len(industries)), 2)}
    ...
    def _fallback_industry_match(self, text: str) -> list:
        ...
        IndustryMatch = namedtuple("IndustryMatch", ["id", "name", "score"])
        results = []
        lower = text.lower()
        for key, data in INDUSTRIES.items():
            ...
            if score > 0:
                results.append(IndustryMatch(id=key, name=data.get("name", key), score=score))
