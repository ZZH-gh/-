...
class PromptToolApp:
    ...
    def __init__(self):
        ...
        # v4 默认启动（D-01: 页面切换模式，v4 对话界面为首屏）
        self._switch_to_v4_page("chat")
        self.set_status("💬 v4.0 对话模式 — 输入需求，我来引导您完善")
    ...
    def _build_header(self):
        ...
        ctk.CTkLabel(self._v3_header, text="v4.0", font=ctk.CTkFont(size=11),
    ...
    def _on_v4_send(self):
        ...
        try:
            ...
            try:
                log_path = os.path.join(os.environ.get("TEMP", os.path.expanduser("~")), "prompt_tool_crash.log")
                with open(log_path, "w", encoding="utf-8") as f:
                    f.write(f"_on_v4_send error: {e}\n{err}")
            except:
                pass
            self._v4_on_error({"error": f"发送出错，详情请查看 %TEMP%\\prompt_tool_crash.log\n\n{str(e)[:200]}"})
    ...
    def _v4_on_question(self, data: dict):
        ...
        question_text = question.get("question_text", data.get("question_text", data.get("message", "")))
    ...
    def _v4_on_error(self, data: dict):
        ...
        self.set_status(f"❌ 出错：{err_msg}")
        # 写错误日志
        try:
            log_path = os.path.join(os.environ.get("TEMP", "."), "prompt_tool_crash.log")
            with open(log_path, "w", encoding="utf-8") as f:
