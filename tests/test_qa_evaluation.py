...
"""
import pytest
from prompt_tool.generator import generate_prompts
from prompt_tool.generator_v2 import generate_prompts_v2
...
class TestQA02Comparison:
    ...
    def test_v4_direct_has_more_content_than_v3(self):
        """v4.0 直给式输出不应比 v3.0 更短（知识注入=更多内容）"""
        v3 = generate_prompts(self.ANALYSIS)
        v4 = generate_prompts_v2(self.ANALYSIS, self.CONTEXT)
        assert len(v4["direct"]) >= len(v3["direct"]) * 0.5  # Allow some variation
